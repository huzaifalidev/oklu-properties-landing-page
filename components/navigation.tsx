      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex-shrink-0">
            <h1 className="text-xl font-bold text-[#d5a86f]">OKLU</h1>
          </div>

          {/* Desktop Menu */}
          <div className="hidden md:flex items-center space-x-8">
            {["home", "about", "investors", "vendors", "testimonials", "contact"].map((item) => (
              <button
                key={item}
                onClick={() => scrollToSection(item)}
                className={`capitalize transition-colors duration-200 font-medium text-sm ${
                  activeSection === item ? "text-[#d5a86f]" : "text-gray-700 hover:text-[#d5a86f]"
                }`}
              >
